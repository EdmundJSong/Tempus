...
const TIMED_TICK_THROTTLE = 0.2; // seconds — fire timedTick callback every ~200ms instead of 50ms

// ============ PRE-RENDERED CLICK BUFFERS ============
// Renders all click variants (6 sounds × 3 levels = 18 buffers) once at init.
// clk() then plays these via createBufferSource().start(time) — one call, no envelope race.

const CLICK_DURATIONS = { sine: 0.08, noise: 0.06, wood: 0.05, rim: 0.05, clave: 0.04, cowbell: 0.1 };
const CLICK_SOUNDS = ["sine", "noise", "wood", "rim", "clave", "cowbell"];

async function renderClick(sampleRate, sound, e) {
  const dur = CLICK_DURATIONS[sound] || 0.1;
  const offCtx = new OfflineAudioContext(1, Math.ceil(sampleRate * dur) + 1, sampleRate);
  const t = 0; // render from time 0

  switch (sound) {
    case "sine": {
      const f = e === 0 ? 1000 : e === 1 ? 750 : 500;
      const v = e === 0 ? 0.8 : e === 1 ? 0.5 : 0.25;
    ...
    case "noise": {
      const l = Math.floor(sampleRate * 0.025);
      const buf = offCtx.createBuffer(1, l, sampleRate);
      const d = buf.getChannelData(0);
      for (let i = 0; i < l; i++) d[i] = Math.random() * 2 - 1;
      const v = e === 0 ? 0.7 : e === 1 ? 0.4 : 0.2;
      const src = offCtx.createBufferSource(), g = offCtx.createGain();
      src.buffer = buf;
      g.gain.setValueAtTime(v, t);
      g.gain.exponentialRampToValueAtTime(0.001, t + 0.05);
      const fl = offCtx.createBiquadFilter();
      fl.type = "bandpass"; fl.frequency.value = e === 0 ? 1200 : e === 1 ? 900 : 700; fl.Q.value = 0.8;
      src.connect(fl); fl.connect(g); g.connect(offCtx.destination);
      src.start(t); src.stop(t + 0.06);
      break;
    }
    case "wood": {
      const v = e === 0 ? 0.9 : e === 1 ? 0.6 : 0.3;
      const o = offCtx.createOscillator(), g = offCtx.createGain();
      o.type = "sine"; o.frequency.value = e === 0 ? 880 : e === 1 ? 780 : 680;
      g.gain.setValueAtTime(v, t);
      g.gain.exponentialRampToValueAtTime(0.001, t + 0.03);
      const fl = offCtx.createBiquadFilter();
      fl.type = "bandpass"; fl.frequency.value = 800; fl.Q.value = 15;
      o.connect(fl); fl.connect(g); g.connect(offCtx.destination);
      o.start(t); o.stop(t + 0.05);
      break;
    }
    case "rim": {
      const v = e === 0 ? 0.85 : e === 1 ? 0.55 : 0.3;
      // Noise layer
      const l = Math.floor(sampleRate * 0.01);
      const buf = offCtx.createBuffer(1, l, sampleRate);
      const d = buf.getChannelData(0);
      for (let i = 0; i < l; i++) d[i] = Math.random() * 2 - 1;
      const src = offCtx.createBufferSource(), g = offCtx.createGain();
      ...
      const fl = offCtx.createBiquadFilter();
      fl.type = "highpass"; fl.frequency.value = 1800; fl.Q.value = 1.2;
      src.connect(fl); fl.connect(g); g.connect(offCtx.destination);
      src.start(t); src.stop(t + 0.05);
      // Sine layer
      const o = offCtx.createOscillator(), g2 = offCtx.createGain();
      o.type = "sine"; o.frequency.value = 1200;
      g2.gain.setValueAtTime(v * 0.6, t);
      g2.gain.exponentialRampToValueAtTime(0.001, t + 0.02);
      o.connect(g2); g2.connect(offCtx.destination);
    ...
    case "clave": {
      const v = e === 0 ? 0.9 : e === 1 ? 0.6 : 0.3;
      const o = offCtx.createOscillator(), g = offCtx.createGain();
      o.type = "sine"; o.frequency.value = e === 0 ? 2500 : e === 1 ? 2300 : 2100;
      g.gain.setValueAtTime(v, t);
    ...
    case "cowbell": {
      const v = e === 0 ? 0.75 : e === 1 ? 0.5 : 0.25;
      const o1 = offCtx.createOscillator(), o2 = offCtx.createOscillator(), g = offCtx.createGain();
      o1.type = "square"; o2.type = "square";
      o1.frequency.value = e === 0 ? 565 : 540;
      o2.frequency.value = e === 0 ? 845 : 800;
      g.gain.setValueAtTime(v, t);
      g.gain.exponentialRampToValueAtTime(0.001, t + 0.08);
      const fl = offCtx.createBiquadFilter();
      fl.type = "bandpass"; fl.frequency.value = 700; fl.Q.value = 3;
      o1.connect(fl); o2.connect(fl); fl.connect(g); g.connect(offCtx.destination);
...
async function renderAllClicks(sampleRate) {
  const bufs = {};
  const promises = [];
  for (const sound of CLICK_SOUNDS) {
    for (const e of [0, 1, 2]) {
      const key = `${sound}_${e}`;
      promises.push(renderClick(sampleRate, sound, e).then(buf => { bufs[key] = buf; }));
    }
  }
...
export function useMetronome(externalCtx) {
  ...
  const clickBufs = useRef(null);   // Fix A: pre-rendered AudioBuffers
  const lastTickCb = useRef(0);      // Fix B: throttle timedTick callbacks
  ...
  // Fix A: clk() now plays pre-rendered buffers instead of building oscillator chains
  const clk = useCallback((ctx, time, bt) => {
    ...
    // Play pre-rendered buffer (1 call instead of ~8)
    const bufs = clickBufs.current;
    if (bufs) {
      const key = `${clickSound}_${e}`;
      const buf = bufs[key];
      if (buf) {
    ...
    // Fallback: real-time synthesis (only if buffers haven't rendered yet)
    switch (clickSound) {
  ...
  const stop = useCallback((token) => {
    ...
    lastTickCb.current = 0;  // Fix B: reset throttle
    rlwl();
    return true;
  }, [rlwl]);
  ...
  const sched = useCallback((token = runId.current) => {
    ...
    while (token === runId.current && pl.current && nb.current < ctx.currentTime + LOOKAHEAD && _guard++ < 200) {
      ...
      if (bar.isT) {
        ...
        // Fix B: throttle timedTick callback — fire every ~200ms instead of every 50ms
        const now = ctx.currentTime;
        if (cbR.current && (now - lastTickCb.current >= TIMED_TICK_THROTTLE)) {
          cbR.current({ type: "timedTick", ab: bar.ab, si: bar.si, el, rem: Math.max(0, bar.tDur - el), dur: bar.tDur });
          lastTickCb.current = now;
        }
        if (el >= bar.tDur) { tsS.current = 0; tsM.current = 0; tsF.current = false; lastTickCb.current = 0; nb.current = Math.max(nb.current, actx.current.currentTime + 0.05); bi.current++; continue; } nb.current += 0.05; return;
      }
  ...
  const start = useCallback((tl, from = 0, ci = 0, s = {}) => {
    stop(); const { syncDelayMs, ...audioSettings } = s; sR.current = { accented: true, clickSound: "sine", muted: false, downbeatOnly: false, silentInterval: 0, ...sR.current, ...audioSettings }; tlR.current = tl; bi.current = from; bei.current = 0; tsS.current = 0; tsM.current = 0; tsF.current = false; lastTickCb.current = 0;
    const ctx = init(); if (ctx.state === "suspended") ctx.resume();
    // Fix A: render click buffers on first start (async, non-blocking — fallback synth covers until ready)
    if (!clickBufs.current) { renderAllClicks(ctx.sampleRate).then(bufs => { clickBufs.current = bufs; }).catch(() => {}); }
  ...
  const tap = useCallback(() => {
    const ctx = init(); if (ctx.state === "suspended") ctx.resume(); prime(ctx);
    // Fix A: pre-render buffers on first tap (so they're ready before first start)
    if (!clickBufs.current) { renderAllClicks(ctx.sampleRate).then(bufs => { clickBufs.current = bufs; }).catch(() => {}); }
    if (!sa.current) { const a = document.createElement("audio"); a.setAttribute("loop", "true"); a.setAttribute("playsinline", "true"); a.src = "data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAAA="; sa.current = a; } try { sa.current.play().catch(() => {}); } catch {} return ctx;
  }, [init, prime]);
