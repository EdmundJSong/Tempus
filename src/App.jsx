...
const C = {bg:"#07070a",surface:"#111116",surfaceHover:"#1a1a22",border:"#25252e",text:"#eeeef0",textMuted:"#6e6e7a",downbeat:"#f0a030",accent:"#8b7cf6",sub:"#3a3a45",danger:"#ef4444",record:"#ef4444",practice:"#22c55e"};
...
// ============ INPUTS ============
function NI({value,onChange,min,max,style={}}){const[d,setD]=useState(String(value));useEffect(()=>setD(String(value)),[value]);const cm=()=>{const v=parseInt(d);if(!isNaN(v)&&v>=min&&v<=max)onChange(v);else setD(String(value));};return<input type="text" inputMode="numeric" value={d} onChange={e=>setD(e.target.value)} onBlur={cm} onKeyDown={e=>{if(e.key==="Enter"){cm();e.target.blur();}}} style={{...nI,...style}}/>;}
...
function StpF({value,onChange,min=0,max=999,step=0.5}){const[d,setD]=useState(String(value));useEffect(()=>setD(String(value)),[value]);const cm=()=>{const v=parseFloat(d);if(!isNaN(v)&&v>=min&&v<=max)onChange(+v.toFixed(1));else setD(String(value));};return(<div style={{display:"flex",alignItems:"center"}}><button onClick={()=>onChange(Math.max(min,+(value-step).toFixed(1)))} style={sB}>{I.chevL(16)}</button><input type="text" inputMode="decimal" value={d} onChange={e=>setD(e.target.value)} onBlur={cm} onKeyDown={e=>{if(e.key==="Enter"){cm();e.target.blur();}}} style={{...nI,width:72}}/><button onClick={()=>onChange(Math.min(max,+(value+step).toFixed(1)))} style={sB}>{I.chevR(16)}</button></div>);}
        ...
        {isMet?(<>
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:18}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              ...
              <NI value={s.tsDen} onChange={v=>{if([1,2,4,8,16,32].includes(v))upd("tsDen",v);}} min={1} max={32} style={{width:56,height:48,fontSize:24,fontWeight:700}}/>
            </div>
...
function PlayView({ps,sections,tl,onPause,onResume,onRestart,onGoToBar,onPrevSec,onNextSec,vis,isP,muted,onMute,onExit,mode,onSplit}){
  ...
  return(
    <div onClick={handleTap} style={{position:"fixed",inset:0,background:C.bg,zIndex:50,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Mono',monospace",boxShadow:borderColor?`inset 0 0 0 4px ${borderColor}, inset 0 0 30px ${borderColor}44`:undefined}}>
      ...
      <div style={{fontFamily:"'Bebas Neue','DM Mono',monospace",fontSize:120,fontWeight:400,color:C.text,lineHeight:1,position:"relative",zIndex:1,letterSpacing:2}}>
      ...
      {mode==="standalone"?(<>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:18}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            ...
            <NI value={tsD} onChange={v=>{if([1,2,4,8,16,32].includes(v))setTsD(v);}} min={1} max={32} style={{width:48,height:42,fontSize:20,fontWeight:700}}/>
          </div>
...
export default function Tempus(){
  ...
  const startPractice=useCallback((standaloneSecs,profileOpts)=>{
    if(standaloneSecs){
      ...
      setTimeout(()=>{const t=buildTL(standaloneSecs);if(t.length)go(0);},100);
    }else if(profileOpts){
      ...
      setTimeout(()=>{const t=buildTL(allSecs);if(t.length)go(0);},100);
    }
  },[sections,go]);
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Outfit',sans-serif",touchAction:"manipulation"}}>
      ...
      <style>{`*{box-sizing:border-box;margin:0;padding:0}html{touch-action:manipulation}input[type=number]::-webkit-inner-spin-button,input[type=number]::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}input[type=number]{-moz-appearance:textfield}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:${C.border};border-radius:2px}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:"12px 16px 24px",background:`linear-gradient(transparent, ${C.bg} 20%)`,display:"flex",justifyContent:"center",gap:16,alignItems:"center",zIndex:10}}>
        <button onClick={()=>{setMode("record");splitPoints.current=[];go(0);}} disabled={!sections.length} style={{width:48,height:48,borderRadius:"50%",background:C.record,border:"none",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 20px ${C.record}44`}}>{I.rec(20)}</button>
        <button onClick={()=>{setMode("normal");go(0);}} disabled={!sections.length} style={{width:64,height:64,borderRadius:"50%",background:C.downbeat,border:"none",color:"#000",cursor:"pointer",boxShadow:`0 0 30px ${C.downbeat}44`,display:"flex",alignItems:"center",justifyContent:"center"}}>{I.play(24)}</button>
        <button onClick={()=>setShowPrac(true)} style={{width:48,height:48,borderRadius:"50%",background:C.practice,border:"none",color:"#000",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 20px ${C.practice}44`}}>{I.target(20)}</button>
      </div>
